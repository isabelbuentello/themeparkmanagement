...
function RideAttendantDash() {
  ...
  const loadDashboard = async () => {
    ...
    try {
      ...
      if (!ridesRes.ok) { setRideError(ridesData.message || 'Failed to load rides'); return }
      if (!queueRes.ok) { setQueueError(queueData.message || 'Failed to load queue overview'); return }

      setRides(ridesData)
      setQueueOverview(queueData)

      const defaults = {}
      ridesData.forEach((ride) => { defaults[ride.ride_id] = ride.status_ride })
  ...
  useEffect(() => {
    loadDashboard()
  }, [statusFilter, rideTypeFilter, rideSort, queueRideFilter])
  ...
  const statusChipStyle = (status) => {
    if (status === 'broken') return { background: 'rgba(220, 38, 38, 0.15)', color: '#b91c1c' }
    if (status === 'maintenance') return { background: 'rgba(245, 158, 11, 0.16)', color: '#b45309' }
    if (status === 'closed_weather') return { background: 'rgba(37, 99, 235, 0.15)', color: '#1d4ed8' }
  ...
  const saveAllStatuses = async () => {
    ...
    const changedRides = rides.filter((ride) => pendingStatus[ride.ride_id] !== originalStatus[ride.ride_id])
    if (changedRides.length === 0) { setRideMessage('No status changes to save'); return }
    ...
    try {
      const results = await Promise.all(
        changedRides.map(async (ride) => {
          ...
          return { ok: res.ok, rideName: ride.ride_name, message: data.message || 'Failed to update status' }
        })
      )
      ...
      if (successCount > 0) setRideMessage('Saved status for ' + successCount + ' ride(s)')
  ...
  const saveFocusStatus = async () => {
    if (!focusRide) { setRideError('Select a ride to save'); return }

    const nextStatus = pendingStatus[focusRide.ride_id] || focusRide.status_ride
    if (nextStatus === originalStatus[focusRide.ride_id]) { setRideMessage('No changes to save for selected ride'); return }
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) { setRideError(data.message || 'Failed to save selected ride'); return }
  ...
  const logRainout = async (rideId) => {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) { setRideError(data.message || 'Failed to log rainout'); return }
  ...
  const viewReservations = async (rideId, rideName) => {
    ...
    try {
      ...
      const data = await res.json()
      if (!res.ok) { setQueueError(data.message || 'Failed to load reservations'); return }
    ...
    <div className="gm-dash-container">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 12, marginBottom: 16 }}>
        {[
          ...
          <article key={item.label} style={{ borderRadius: 14, border: '1px solid rgba(148, 163, 184, 0.22)', background: 'linear-gradient(180deg, rgba(255,255,255,0.97), rgba(248,250,252,0.94))', boxShadow: '0 8px 20px rgba(15, 23, 42, 0.08)', padding: '14px 14px 12px' }}>
            <div style={{ display: 'inline-block', padding: '3px 9px', borderRadius: 999, background: item.tint, color: item.tone, fontSize: '0.74rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em' }}>
              {item.label}
            </div>
            <div style={{ marginTop: 10, fontSize: '2rem', lineHeight: 1.1, fontWeight: 800, color: '#0f172a' }}>
              {item.value}
            </div>
          </article>
        ))}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 12, marginBottom: 16 }}>
        <article style={{ borderRadius: 16, border: '1px solid rgba(148, 163, 184, 0.24)', background: 'linear-gradient(160deg, rgba(255,255,255,0.98), rgba(248,250,252,0.94))', boxShadow: '0 10px 24px rgba(15, 23, 42, 0.08)', padding: '14px 16px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', marginBottom: 10 }}>
            <h3 style={{ margin: 0 }}>Rides Needing Attention</h3>
            <span style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minWidth: 34, height: 28, padding: '0 8px', borderRadius: 999, background: 'rgba(220, 38, 38, 0.12)', color: '#b91c1c', fontWeight: 800, fontSize: '0.9rem' }}>
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {attentionRides.slice(0, 6).map((ride) => (
              <div key={ride.ride_id} style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', padding: '8px 10px', borderRadius: 10, background: 'rgba(241, 245, 249, 0.8)', border: '1px solid rgba(148, 163, 184, 0.2)' }}>
                <strong style={{ color: '#0f172a' }}>{ride.ride_name}</strong>
                <span style={{ ...statusChipStyle(ride.status_ride), display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minWidth: 118, padding: '4px 10px', borderRadius: 999, fontSize: '0.76rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', whiteSpace: 'nowrap' }}>
      ...
      <div style={{ borderRadius: 16, border: '1px solid rgba(148, 163, 184, 0.24)', background: 'linear-gradient(145deg, rgba(255,255,255,0.98), rgba(248,250,252,0.94))', boxShadow: '0 10px 24px rgba(15, 23, 42, 0.08)', padding: '14px 16px', marginBottom: 16 }}>
        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
          <label htmlFor="focus-ride-select" style={{ fontWeight: 700, color: '#0f172a' }}>Ride</label>
          <select id="focus-ride-select" value={focusRideId} onChange={(e) => setFocusRideId(e.target.value)}>
            {rides.map((ride) => (
          ...
          {focusRide && (
            <>
              <button onClick={() => setPendingStatus((prev) => ({ ...prev, [focusRide.ride_id]: 'open' }))} style={{ background: '#166534', color: '#fff' }}>Mark Open</button>
              <button onClick={() => setPendingStatus((prev) => ({ ...prev, [focusRide.ride_id]: 'maintenance' }))} style={{ background: '#b45309', color: '#fff' }}>Mark Maintenance</button>
              <button onClick={() => setPendingStatus((prev) => ({ ...prev, [focusRide.ride_id]: 'broken' }))} style={{ background: '#b91c1c', color: '#fff' }}>Mark Broken</button>
              <button onClick={saveFocusStatus} disabled={!focusHasDraftChange || isSavingFocus} style={{ background: '#1d4ed8', color: '#fff' }}>
        ...
        {focusRide && (
          <div style={{ marginTop: 12 }}>
            <div style={{ display: 'inline-flex', alignItems: 'center', justifyContent: 'center', minWidth: 128, padding: '5px 12px', borderRadius: 999, fontSize: '0.78rem', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.04em', whiteSpace: 'nowrap', ...statusChipStyle(focusRide.status_ride) }}>
